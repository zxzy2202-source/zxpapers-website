  ...
  async function importCampaign() {
    ...
    try {
      const response = await fetch("/api/admin/posts/campaign", {
        ...
        body: JSON.stringify({ campaignId, startAt: new Date(startAt).toISOString() }),
      });
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-slate-900">Blog 内容排期导入</h2>
          </div>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
            选择一组已写好的英文成稿按周导入后台。导入后默认处于待审核状态，只有在文章编辑页勾选“批准自动发布”后，定时任务才会公开文章。
          </p>
          {selectedCampaign ? (
            <p className="mt-2 inline-flex items-center gap-1.5 text-sm font-medium text-emerald-700">
              <CheckCircle2 className="h-4 w-4" aria-hidden="true" /> 已导入 {selectedCampaign.imported}/{selectedCampaign.total} 篇
            </p>
          ) : null}
          {selectedCampaign?.description ? (
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">{selectedCampaign.description}</p>
          ) : null}
        </div>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
          <label className="text-sm font-medium text-slate-700">
            ...
            <select
              ...
              className="mt-1 block min-h-10 min-w-64 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
            >
              {campaigns.map((campaign) => (
                <option key={campaign.id} value={campaign.id}>
                  {campaign.name} ({campaign.total} 篇，每 {campaign.cadenceDays} 天)
                </option>
              ))}
            </select>
          </label>
          ...
          <button
            ...
            disabled={loading || !selectedCampaign || selectedCampaign.imported >= selectedCampaign.total || !startAt}
          ...
          >
            ...
            {selectedCampaign && selectedCampaign.imported >= selectedCampaign.total ? "已全部导入" : "导入排期"}
          </button>
        </div>
      </div>
