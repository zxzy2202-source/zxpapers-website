...
import { TrendingUp, Lightbulb, Clock, ChevronRight } from "lucide-react";
    ...
    <Layout>
      <div className="bg-[#0F2B5B] text-white py-14">
        <div className="container">
          ...
          <p className="text-slate-300 max-w-2xl text-base">Market analysis, regulatory updates, and strategic insights for thermal paper distributors, buyers, and brand owners.</p>
        </div>
      </div>
      ...
      <div className="container py-14">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-8">
            {posts.length > 0 ? (
              posts.map((post) => (
                <article key={post._id} className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="p-6 flex flex-col justify-between">
                      <div>
                        ...
                        <h2 className="font-sora text-xl font-bold text-slate-900 mb-2 hover:text-[#0F2B5B] transition-colors">
                          <Link href={`/resources/industry-insights/${post.slug.current}`}>{post.title}</Link>
                        </h2>
                      ...
                      <Link 
                        href={`/resources/industry-insights/${post.slug.current}`}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-50 rounded-2xl p-5">
              ...
              <ul className="space-y-2">
                {[{ label: "OEM Guide", href: "/resources/oem-guide" }, { label: "Product Knowledge", href: "/resources/product-knowledge" }, { label: "Application Cases", href: "/resources/application-cases" }].map(({ label, href }) => (
                  <li key={href}><Link href={href} className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 transition-colors"><ChevronRight className="w-3.5 h-3.5" />{label}</Link></li>
                ))}
              </ul>
            </div>
          </div>
